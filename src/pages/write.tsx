...
export default class Write extends Component<{}, WriteState> {
  ...
  render() {
    const { value, isClient, showAssistant } = this.state;
    ...
    return (
      <div className="bg-bgmain flex flex-col min-h-screen w-screen gap-14 relative">
        ...
        <div className="flex flex-row justify-start mt-40 lg:mt-40 md:mt-40 sm:mt-40 px-20">
          {isClient ? (
            <ReactQuill
              theme="bubble"
              value={value}
          ...
          <div>
            <div className={`${showAssistant ? "hidden" : "block"} text-black px-14 py-3 bg-secondary-300 h-fit rounded-xl`}
            onClick={assistFunc}>Start Assistant</div>
          </div>
          {/* <div className='w-full text-2xl text-black'>word count: </div> */}
        </div>
